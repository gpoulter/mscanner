...
class MedscanTests(unittest.TestCase):
    def test(self):
        serv = MScannerService()
        #self.assertEqual(listBatches(), [])
        self.assertRaises(Fault, serv.deleteBatch, "noexist")
        self.assertRaises(Fault, serv.deleteBatch, "11223344-112233")
        self.assertEqual(serv.getStatus("11223344-112233"), dict(batchid="11223344-112233", status="notfound"))
        # Test query
        batchid = serv.query("11809184\n12069159\n9744524\n1960624\n", 0.1, 5)
        time.sleep(2)
        self.assertEqual(serv.getStatus(batchid)["status"], "busy")
        id, status = os.waitpid(int(pidfile.lines()[0].strip()),0)
        self.assertEqual(serv.getStatus(batchid), dict(batchid=batchid, status="done"))
        self.assertEqual(serv.getStatus(""), dict(status="free"))
        deleteBatch(batchid)
        # Test validation
        batchid = serv.validate("11809184\n12069159\n9744524\n1960624\n", 50, 5, 0.9, 0.1)
        time.sleep(1)
        self.assertEqual(serv.getStatus(batchid)["status"], "busy")
        id, status = os.waitpid(int(pidfile.lines()[0].strip()),0)
        self.assertEqual(serv.getStatus(batchid), dict(batchid=batchid, status="done"))
